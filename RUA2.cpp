...
void menu_administrador() {
    ...
    if (login!=loginReal){
        while (contador<3){
        printf("LOGIN INVÁLIDO!\nINSIRA SEU LOGIN NOVAMENTE!\n\n");
        scanf("%i",&login);
        contador++;
        }
        printf("INFORMAÇÕES DE LOGIN DIGITADAS INCORRETAMENTE POR MAIS DE 3 VEZES CONSECUTIVAS\n");
    ...
    fflush(stdin);
    if (senha!=senhaReal){
        while (contadorSenha<3){
        printf("SENHA INVÁLIDA!\n");
        printf("INSIRA SUA SENHA NOVAMENTE\n");
        scanf("%i",&senha);
        contadorSenha++;
        }
        printf("SENHA DIGITADA INCORRETAMENTE POR MAIS DE 3 VEZES CONSECUTIVAS\n\n");
        printf("RETORNANDO AO MENU PRINCIPAL\n\n");
        menu_principal();
    }
    }
